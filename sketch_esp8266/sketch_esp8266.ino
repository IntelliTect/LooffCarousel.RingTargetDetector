...
// however while this worked well with sensors in a flat position, in a vertical position, sensor stuck in a high state now are observed
// to get stuck and take a real smacking to go low. and so checkVibrationSensorsAreDifferentOrBothLow was implemented as a quick hack to get the display going on install day.
// So choose wither checkVibrationSensorsAreDifferentOrBothLow, or checkVibrationSensorsAreDifferent
bool doubleCheckSensors() {
  ...
  return checkVibrationSensorsAreDifferentOrBothLow(_vibReading, _vibReading2);
}

bool checkVibrationSensorsAreDifferentOrBothLow(uint8_t reading1, uint8_t reading2) {

  //
  if (reading1 != reading2) {
    return true;
  }
  return checkVibrationSensorsForLow(reading1, reading2);
}

bool checkVibrationSensorsForLow(uint8_t reading1, uint8_t reading2) {
